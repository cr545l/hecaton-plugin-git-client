...
// 붙고, 끝나면 브랜치명부터 시작하는 평소 타이틀로 돌아와야 한다.
...
test('유휴 상태의 타이틀은 브랜치명부터 시작한다', () => {
  resetState();
  assert.equal(formatWindowTitle(), 'main | *1 | ↓1 | ↑2');
});
...
test('쓰기 작업 중에는 브랜치명 앞에 스피너와 진행 메시지가 붙는다', () => {
  ...
  assert.equal(title, BRAILLE_FRAMES[2] + ' Committing... | main | *1 | ↓1 | ↑2');
});
...
test('백그라운드 refresh도 타이틀에서 표시한다', () => {
  ...
  assert.match(formatWindowTitle(), /^⠋ Scanning repository\.\.\. \| main/);
});
